...
// LOG_MODULE_REGISTER(ecc_make_key,  LOG_LEVEL_DBG);
// LOG_MODULE_REGISTER(EccPoint_mult, LOG_LEVEL_DBG);
...
uint64_t Curve_P_16[2] = {0xFFFFFFFFFFFFFFFF, 0xFFFFFFFDFFFFFFFF};
uint64_t Curve_P_24[3] = {0xFFFFFFFFFFFFFFFFull, 0xFFFFFFFFFFFFFFFEull, 0xFFFFFFFFFFFFFFFFull};
uint64_t Curve_P_32[4] = {0xFFFFFFFFFFFFFFFFull, 0x00000000FFFFFFFFull, 0x0000000000000000ull, 0xFFFFFFFF00000001ull};
uint64_t Curve_P_48[6] = {0x00000000FFFFFFFF, 0xFFFFFFFF00000000, 0xFFFFFFFFFFFFFFFE, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF};

uint64_t Curve_B_16[2] = {0xD824993C2CEE5ED3, 0xE87579C11079F43D};
uint64_t Curve_B_24[3] = {0xFEB8DEECC146B9B1ull, 0x0FA7E9AB72243049ull, 0x64210519E59C80E7ull};
uint64_t Curve_B_32[4] = {0x3BCE3C3E27D2604Bull, 0x651D06B0CC53B0F6ull, 0xB3EBBD55769886BCull, 0x5AC635D8AA3A93E7ull};
uint64_t Curve_B_48[6] = {0x2A85C8EDD3EC2AEF, 0xC656398D8A2ED19D, 0x0314088F5013875A, 0x181D9C6EFE814112, 0x988E056BE3F82D19, 0xB3312FA7E23EE7E4};

uint64_t Curve_G_x_16[2] = {0x0C28607CA52C5B86, 0x161FF7528B899B2D};
uint64_t Curve_G_y_16[2] = {0xC02DA292DDED7A83, 0xCF5AC8395BAFEB13};
EccPoint Curve_G_16 = {Curve_G_x_16, Curve_G_y_16};

uint64_t Curve_G_x_24[3] = {0xF4FF0AFD82FF1012ull, 0x7CBF20EB43A18800ull, 0x188DA80EB03090F6ull};
uint64_t Curve_G_y_24[3] = {0x73F977A11E794811ull, 0x631011ED6B24CDD5ull, 0x07192B95FFC8DA78ull};
EccPoint Curve_G_24 = {Curve_G_x_24, Curve_G_y_24};

uint64_t Curve_G_x_32[4] = {0xF4A13945D898C296ull, 0x77037D812DEB33A0ull, 0xF8BCE6E563A440F2ull, 0x6B17D1F2E12C4247ull};
uint64_t Curve_G_y_32[4] = {0xCBB6406837BF51F5ull, 0x2BCE33576B315ECEull, 0x8EE7EB4A7C0F9E16ull, 0x4FE342E2FE1A7F9Bull};
EccPoint Curve_G_32 = {Curve_G_x_32, Curve_G_y_32};

uint64_t Curve_G_x_48[6] = {0x3A545E3872760AB7, 0x5502F25DBF55296C, 0x59F741E082542A38, 0x6E1D3B628BA79B98, 0x8EB1C71EF320AD74, 0xAA87CA22BE8B0537};
uint64_t Curve_G_y_48[6] = {0x7A431D7C90EA0E5F, 0x0A60B1CE1D7E819D, 0xE9DA3113B5F0B8C0, 0xF8F41DBD289A147C, 0x5D9E98BF9292DC29, 0x3617DE4A96262C6F};
EccPoint Curve_G_48 = {Curve_G_x_48, Curve_G_y_48};
    

uint64_t Curve_N_16[2] = {0x75A30D1B9038A115, 0xFFFFFFFE00000000};
uint64_t Curve_N_24[3] = {0x146BC9B1B4D22831ull, 0xFFFFFFFF99DEF836ull, 0xFFFFFFFFFFFFFFFFull};
uint64_t Curve_N_32[4] = {0xF3B9CAC2FC632551ull, 0xBCE6FAADA7179E84ull, 0xFFFFFFFFFFFFFFFFull, 0xFFFFFFFF00000000ull};
uint64_t Curve_N_48[6] = {0xECEC196ACCC52973, 0x581A0DB248B0A77A, 0xC7634D81F4372DDF, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF};


static uint64_t *curve_p;
static uint64_t *curve_b;
static EccPoint curve_G;
static uint64_t *curve_n;
...
static int getRandomNumber(uint64_t *p_vli)
{
    ...
    CryptGenRandom(l_prov, CT, (BYTE *)p_vli);
...
int set_CT(curve_type CT_val)
{
    CT = CT_val;
    switch (CT_val)
    {
    case secp192r1:
        curve_p = &Curve_P_24[0];
        curve_b = &Curve_B_24[0];
        curve_G = Curve_G_24;
        curve_n = &Curve_N_24[0];
        break;
    case secp256r1:
        curve_p = &Curve_P_32[0];
        curve_b = &Curve_B_32[0];
        curve_G = Curve_G_32;
        curve_n = &Curve_N_32[0];
        break;
...
static void vli_mult(uint64_t *p_result, uint64_t *p_left, uint64_t *p_right)
{
    ...
    for(k=0; k < CT/8*2 - 1; ++k)
    {
        uint l_min = (k < CT/8 ? 0 : (k + 1) - CT/8);
        for(i=l_min; i<=k && i<CT/8; ++i)
        {
    ...
    p_result[CT/8*2 - 1] = (uint64_t)r01;
}
...
static void omega_mult(uint64_t *p_result, uint64_t *p_right)
{
    ...
    uint64_t *l_tmp = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t l_carry, l_diff;
    
    /* Multiply by (2^128 + 2^96 - 2^32 + 1). */
    vli_set(p_result, p_right); /* 1 */
    l_carry = vli_lshift(l_tmp, p_right, 32);
    p_result[1 + CT/8] = l_carry + vli_add(p_result + 1, p_result + 1, l_tmp); /* 2^96 + 1 */
    p_result[2 + CT/8] = vli_add(p_result + 2, p_result + 2, p_right); /* 2^128 + 2^96 + 1 */
    l_carry += vli_sub(p_result, p_result, l_tmp); /* 2^128 + 2^96 - 2^32 + 1 */
    l_diff = p_result[CT/8] - l_carry;
    if(l_diff > p_result[CT/8])
...
static void vli_mmod_fast(uint64_t *p_result, uint64_t *p_product)
{
    ...
    case secp384r1:
        {
            while(!vli_isZero(p_product + CT/8)) /* While c1 != 0 */
            {
                uint64_t l_carry = 0;
                uint i;
                
                vli_clear(l_tmp);
                vli_clear(l_tmp + CT/8);
                omega_mult(l_tmp, p_product + CT/8); /* tmp = w * c1 */
                vli_clear(p_product + CT/8); /* p = c0 */
...
static void vli_modMult_fast(uint64_t *p_result, uint64_t *p_left, uint64_t *p_right)
{
    ...
    uint64_t *l_product = (uint64_t *)malloc(CT/8 * 2 * sizeof(uint64_t));
...
static void vli_modInv(uint64_t *p_result, uint64_t *p_input, uint64_t *p_mod)
{
    ...
    uint64_t *a = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)),
             *b = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)),
             *u = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)),
             *v = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t l_carry;
    int l_cmpResult;
    ...
    while((l_cmpResult = vli_cmp(a, b)) != 0)
    {
        ...
        if(EVEN(a))
        {
            ...
            if(l_carry)
            {
                u[CT/8-1] |= 0x8000000000000000ull;
            }
        }
        else if(EVEN(b))
        {
            ...
            if(l_carry)
            {
                v[CT/8-1] |= 0x8000000000000000ull;
            }
        }
...
static void EccPoint_double_jacobian(uint64_t *X1, uint64_t *Y1, uint64_t *Z1)
{
    ...
    uint64_t *t4 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t *t5 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
...
static void apply_z(uint64_t *X1, uint64_t *Y1, uint64_t *Z)
{
    ...
    uint64_t *t1 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
...
{
    uint64_t *z = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    vli_set(X2, X1);
    vli_set(Y2, Y1);
...
static void XYcZ_addC(uint64_t *X1, uint64_t *Y1, uint64_t *X2, uint64_t *Y2)
{
    ...
    uint64_t *t5 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t *t6 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t *t7 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
...
static void EccPoint_mult(EccPoint *p_result, EccPoint *p_point, uint64_t *p_scalar, uint64_t *p_initialZ)
{
    ...
    LOG_MODULE_DECLARE(EccPoint_mult, CONFIG_LOG_DEFAULT_LEVEL);
    /* R0 and R1 */
    // uint64_t Rx[2][NUM_ECC_DIGITS];
    // uint64_t Ry[2][NUM_ECC_DIGITS];
    // uint64_t z[NUM_ECC_DIGITS];
    uint64_t *Rx[2];
    Rx[0] = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    Rx[1] = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t *Ry[2];
    Ry[0] = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    Ry[1] = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t *z = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint8_t i, nb;
...
static void ecc_bytes2native(uint64_t *p_native, const uint8_t *p_bytes)
{
    unsigned i;
    for(i=0; i<CT/8; ++i)
    {
        const uint8_t *p_digit = p_bytes + 8 * (CT/8 - 1 - i);
...
static void ecc_native2bytes(uint8_t *p_bytes, const uint64_t *p_native)
{
    unsigned i;
    for(i=0; i<CT/8; ++i)
    {
        uint8_t *p_digit = p_bytes + 8 * (CT/8 - 1 - i);
...
static void mod_sqrt(uint64_t *a)
{
    ...
    uint64_t *p1 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t *l_result = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    p1[0]=1;
    l_result[0]=1;
...
static void ecc_point_decompress(EccPoint *p_point, const uint8_t *p_compressed)
{
    // uint64_t _3[NUM_ECC_DIGITS] = {3}; /* -a = 3 */
    uint64_t *_3 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    _3[0] = 3;
...
int ecc_make_key(uint8_t *p_publicKey, uint8_t *p_privateKey)
{
    LOG_MODULE_DECLARE(ecc_make_key, CONFIG_LOG_DEFAULT_LEVEL);

    uint64_t *l_private = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    EccPoint l_public = {
        .x = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)),
        .y = (uint64_t *)malloc(CT/8 * sizeof(uint64_t))
    };
    // unsigned l_tries = 0;
    LOG_DBG("ecc_make_key %u", CT/8 * sizeof(uint64_t));
    do
    {
        sys_rand_get(l_private, CT/8 * sizeof(uint64_t));
        LOG_DBG("sys_rand_get %u", CT/8 * sizeof(uint64_t));
        // if(!getRandomNumber(l_private) || (l_tries++ >= MAX_TRIES))
        // {
        //     printf("getRandomNumber failed\n");
        //     return 0;
        // }
...
int ecdh_shared_secret(const uint8_t *p_publicKey, const uint8_t *p_privateKey, uint8_t *p_secret)
{
    // EccPoint l_public;
    // uint64_t l_private[NUM_ECC_DIGITS];
    // uint64_t l_random[NUM_ECC_DIGITS];
    uint64_t *l_private = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t *l_random = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    EccPoint l_public = {
        .x = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)),
        .y = (uint64_t *)malloc(CT/8 * sizeof(uint64_t))
    };
    
    sys_rand_get(l_random, CT/8 * sizeof(uint64_t));
    ...
    int ret = EccPoint_isZero(&l_product);
    free(l_private);
    free(l_random);
    free(l_public.x);
    free(l_public.y);
...
static void vli_modMult(uint64_t *p_result, uint64_t *p_left, uint64_t *p_right, uint64_t *p_mod)
{
    ...
    uint64_t *l_product = (uint64_t *)malloc(CT/8 * 2 * sizeof(uint64_t));
    uint64_t *l_modMultiple = (uint64_t *)malloc(CT/8 * 2 * sizeof(uint64_t));
    uint l_digitShift, l_bitShift;
    uint l_productBits;
    uint l_modBits = vli_numBits(p_mod);
    
    vli_mult(l_product, p_left, p_right);
    l_productBits = vli_numBits(l_product + CT/8);
    if(l_productBits)
    {
        l_productBits += CT/8 * 64;
    }
    else
    {
    ...
    vli_clear(l_modMultiple + CT/8);
    l_digitShift = (l_productBits - l_modBits) / 64;
    l_bitShift = (l_productBits - l_modBits) % 64;
    if(l_bitShift)
    {
        l_modMultiple[l_digitShift + CT/8] = vli_lshift(l_modMultiple + l_digitShift, p_mod, l_bitShift);
    }
    else
    {
    ...
    while(l_productBits > CT/8 * 64 || vli_cmp(l_modMultiple, p_mod) >= 0)
    {
        int l_cmp = vli_cmp(l_modMultiple + CT/8, l_product + CT/8);
        if(l_cmp < 0 || (l_cmp == 0 && vli_cmp(l_modMultiple, l_product) <= 0))
        {
            if(vli_sub(l_product, l_product, l_modMultiple))
            { /* borrow */
                vli_sub(l_product + CT/8, l_product + CT/8, p_result);
            }
            vli_sub(l_product + CT/8, l_product + CT/8, l_modMultiple + CT/8);
        }
        uint64_t l_carry = (l_modMultiple[CT/8] & 0x01) << 63;
        vli_rshift1(l_modMultiple + CT/8);
        vli_rshift1(l_modMultiple);
        l_modMultiple[CT/8-1] |= l_carry;
...
int ecdsa_sign(const uint8_t *p_privateKey, const uint8_t *p_hash, uint8_t *p_signature)
{
    // uint64_t k[NUM_ECC_DIGITS];
    // uint64_t l_tmp[NUM_ECC_DIGITS];
    // uint64_t l_s[NUM_ECC_DIGITS];
    uint64_t* k = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t* l_tmp = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    uint64_t* l_s = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    EccPoint p = {
        .x = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)),
        .y = (uint64_t *)malloc(CT/8 * sizeof(uint64_t))
    };
    ...
    do
    {
        sys_rand_get(k, CT/8 * sizeof(uint64_t));
    ...
    ecc_native2bytes(p_signature + CT, l_s);
    free(k);
    free(l_tmp);
    free(l_s);
    free(p.x);
...
int ecdsa_verify(const uint8_t *p_publicKey, const uint8_t *p_hash, const uint8_t *p_signature)
{
    ...
    uint64_t *u1 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *u2 = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *z = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *rx = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *ry = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *tx = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *ty = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *tz = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *l_r = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)), 
             *l_s = (uint64_t *)malloc(CT/8 * sizeof(uint64_t));
    EccPoint l_public = {
                .x = (uint64_t *)malloc(CT/8 * sizeof(uint64_t)),
                .y = (uint64_t *)malloc(CT/8 * sizeof(uint64_t))
            }, 
    ...
    ecc_bytes2native(l_s, p_signature + CT);
